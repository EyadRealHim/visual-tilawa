...
def verse_info(key: VerseKey):
    ...
    for raw_word in verse_data["words"]:
        ...
        segment = audio_segments[-1] if position >= len(audio_segments) else audio_segments[position]
...
def extract_clips(verse: VerseInformation):
    ...
    silence_periods = silence.detect_silence(
        audio_segment=audio, min_silence_len=350, silence_thresh=audio.dBFS - SILENCE_THRESHOLD
    )

    silence_periods = [(a + b) // 2 for a, b in silence_periods]
    silence_periods = [(prev, curr) for prev, curr in zip([0] + silence_periods, silence_periods)]
    ...
    for index, (begin, end) in enumerate(silence_periods):
        ...
        clips.append(
            ClipInformation(
                verse_key=verse.verse_key, clip_index=index, 
                content=collection, audio_url=verse.audio_url,
                begin=begin, end=end
            )
        )

    if words:
        raise RuntimeError(f"Failed to group verse words into clips. {len(words)} words left unorganized.")
